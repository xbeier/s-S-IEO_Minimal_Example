...
####################################### LABELLING (see Bubak et al. (2026) for details)
...
def compute_labels(D: np.array, gamma: float, K: float, c: float, b: float, h: float, bounds: tuple, num: int=20000, step_initguess: int=5):
    ...
    for t in reversed(range(last_period+1)):
        ...
        # get (s,S) estimates
        s_lower, S0, _ = discrete_min_kconvex(G=Gt, S_upper=high, K=K, step=step_initguess)
        ...
        s_tp1 = brentq(f=root_fun, a=s_lower-step_initguess, b=S)
